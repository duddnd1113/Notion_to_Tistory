...
2. 변환할 노션 페이지 URL 붙여넣기
3. 변환 옵션 선택 후 **변환하기**
4. 미리보기 확인 후 **HTML 복사**
5. 티스토리 글쓰기 → 우측 상단 `</>` HTML 버튼 클릭 → **붙여넣기**
...
- **토큰은 브라우저 `localStorage`에만 저장**되며 외부 서버로 전송되지 않습니다.
- 노션 이미지는 S3 임시 URL이므로 **만료 전에 티스토리에 직접 업로드**하는 것을 권장합니다.
